...
def main():
    ...
    print("=" * 60)
    print("Launching Overhauled JAX-Native Curriculum Gate Navigation (v2)")
    print("=" * 60)
    ...
    # --- CURRICULUM OPTIMIZED TO PREVENT OVERFITTING ---
    curriculum_stages = [
        {"level": 1, "steps": 3_000_000, "lr": 3.0e-4, "entropy": 1.0e-3},
        {"level": 2, "steps": 5_000_000, "lr": 2.5e-4, "entropy": 5.0e-4},   # Cut from 12M to 5M (peaks at 4.9M)
        {"level": 3, "steps": 8_000_000, "lr": 2.0e-4, "entropy": 5.0e-4},   # Cut from 15M to 8M
        {"level": 4, "steps": 10_000_000, "lr": 1.0e-4, "entropy": 1.0e-4},  # Cut from 20M to 10M
        {"level": 5, "steps": 15_000_000, "lr": 3.0e-4, "entropy": 5.0e-3},  # Cut from 25M to 15M
    ]
    ...
    for stage_idx, stage in enumerate(curriculum_stages):
        ...
        print(f"\n" + "#" * 60)
        print(f"STARTING CURRICULUM STAGE {level} ({steps} steps, LR: {lr}, Entropy: {entropy})")
        print("#" * 60)
        ...
        make_inference_fn, params, _ = ppo.train(
            ...
            # --- BALANCED 16GB VRAM CONFIG ---
            num_envs=1024,               # High enough for good batch diversity, safe for 16GB
            unroll_length=128,           # 128 steps per env
            num_minibatches=32,          # (1024 * 128) / 32 = batch size of 4096
            num_updates_per_batch=4,     # Standard stable PPO optimization passes
