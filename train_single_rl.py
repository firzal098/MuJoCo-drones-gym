...
class SingleGateTrainingWrapper(gym.Wrapper):
    ...
    def __init__(self, env, rank=0):
        ...
        self.nominal_gate_pos = np.array([0.17, 9.26, 1.0])
        self.gate_targets = [
        ...
        # Observations: [YOLO_Box(4), Rel_Noisy_Gate(3), EKF_Vel(3), Attitude(3)] = 13 Dimensions
    ...
    def reset(self, seed=None, options=None):
        ...
        # Randomize gate offset by up to +/- 80cm in X and Y
        gx = np.random.uniform(-0.8, 0.8)
        gy = np.random.uniform(-0.8, 0.8)
        
        body_id = mujoco.mj_name2id(self.env.model, mujoco.mjtObj.mjOBJ_BODY, "gate_single_a")
        if body_id >= 0:
            self.env.model.body_pos[body_id] = np.array([0.17 + gx, 9.26 + gy, 0.0])
    ...
    def step(self, action):
        ...
        # Map actions: [vx_body, vy_body, vz_body_down, yaw_rate] (Option B)
        vx_body = action[0] * max_speed
        vy_body = action[1] * max_speed
        # vz_body is positive down in MAVLink body frame, but positive up in GuidedVehicle
