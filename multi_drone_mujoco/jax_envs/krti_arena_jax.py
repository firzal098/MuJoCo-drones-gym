import jax
import mujoco
import numpy as np
from brax.envs.base import Env, State
from jax import numpy as jnp
from mujoco import mjx

from multi_drone_mujoco.jax_control.guided_mode_jax import (
    init_controller_state,
    update_controller,
)
from multi_drone_mujoco.jax_envs.base_aviary_jax import BaseAviaryJax
from multi_drone_mujoco.utils.enums import DroneModel


def get_gate_corners_jax(gate_type="single"):
    x_min, x_max = 0.07, 1.83
    z_min, z_max = 0.20, 1.85
    t = 0.08
    y_extents = [-t, t]

    corners = []
    for dx in [x_min, x_max]:
        for dy in y_extents:
            for dz in [z_min, z_max]:
                corners.append(jnp.array([dx, dy, dz]))
    return jnp.stack(corners)

def compute_fake_yolo_jax(gate_pos, gate_mat, cam_pos, cam_mat, fovy, width, height):
    """JAX implementation of 3D to 2D camera projection for YOLO bounding box."""
    corners = get_gate_corners_jax()

    def project_pt(pt_local):
        pt_world = jnp.dot(gate_mat, pt_local) + gate_pos
        dp = pt_world - cam_pos
        p_cam = jnp.dot(cam_mat.T, dp)
        x_c, y_c, z_c = p_cam[0], p_cam[1], p_cam[2]

        depth = -z_c
        f_y = 1.0 / jnp.tan(jnp.deg2rad(fovy) / 2.0)
        f_x = f_y * (height / width)

        ndc_x = f_x * (x_c / depth)
        ndc_y = f_y * (y_c / depth)

        px_x = (ndc_x + 1.0) / 2.0 * width
        px_y = (1.0 - ndc_y) / 2.0 * height

        valid = z_c < 0
        return jnp.array([px_x, px_y]), valid

    pts, valids = jax.vmap(project_pt)(corners)
    any_valid = jnp.any(valids)

    xs = jnp.where(valids, pts[:, 0], jnp.inf)
    ys = jnp.where(valids, pts[:, 1], jnp.inf)

    unclipped_xmin = jnp.min(xs)
    unclipped_ymin = jnp.min(ys)

    xs_max = jnp.where(valids, pts[:, 0], -jnp.inf)
    ys_max = jnp.where(valids, pts[:, 1], -jnp.inf)

    unclipped_xmax = jnp.max(xs_max)
    unclipped_ymax = jnp.max(ys_max)

    outside = (unclipped_xmin > width) | (unclipped_xmax < 0.0) | (unclipped_ymin > height) | (unclipped_ymax < 0.0)
    visible = any_valid & (~outside)

    x_min = jnp.clip(unclipped_xmin / width, 0.0, 1.0)
    y_min = jnp.clip(unclipped_ymin / height, 0.0, 1.0)
    x_max = jnp.clip(unclipped_xmax / width, 0.0, 1.0)
    y_max = jnp.clip(unclipped_ymax / height, 0.0, 1.0)

    valid_box = jnp.array([x_min, y_min, x_max, y_max])
    invalid_box = jnp.array([-1.0, -1.0, -1.0, -1.0])

    return jnp.where(visible, valid_box, invalid_box)

def compute_unbounded_bbox_jax(gate_pos, gate_mat, cam_pos, cam_mat, fovy, width=320, height=240):
    corners = get_gate_corners_jax()

    def project_pt(pt_local):
        pt_world = jnp.dot(gate_mat, pt_local) + gate_pos
        dp = pt_world - cam_pos
        p_cam = jnp.dot(cam_mat.T, dp)
        x_c, y_c, z_c = p_cam[0], p_cam[1], p_cam[2]

        # Prevent division by zero
        depth = jnp.maximum(-z_c, 1e-3)
        f_y = 1.0 / jnp.tan(jnp.deg2rad(fovy) / 2.0)
        f_x = f_y * (height / width)

        ndc_x = f_x * (x_c / depth)
        ndc_y = f_y * (y_c / depth)

        return jnp.array([ndc_x, ndc_y]), z_c < 0

    pts, valids = jax.vmap(project_pt)(corners)
    any_valid = jnp.any(valids)

    xs = jnp.where(valids, pts[:, 0], jnp.inf)
    ys = jnp.where(valids, pts[:, 1], jnp.inf)
    unclipped_xmin = jnp.min(xs)
    unclipped_ymin = jnp.min(ys)

    xs_max = jnp.where(valids, pts[:, 0], -jnp.inf)
    ys_max = jnp.where(valids, pts[:, 1], -jnp.inf)
    unclipped_xmax = jnp.max(xs_max)
    unclipped_ymax = jnp.max(ys_max)

    u_c = jnp.where(any_valid, (unclipped_xmin + unclipped_xmax) / 2.0, 0.0)
    v_c = jnp.where(any_valid, (unclipped_ymin + unclipped_ymax) / 2.0, 0.0)
    w_b = jnp.where(any_valid, unclipped_xmax - unclipped_xmin, 0.0)
    h_b = jnp.where(any_valid, unclipped_ymax - unclipped_ymin, 0.0)

    # Clip centers to prevent reward explosion near depth zero
    u_c = jnp.clip(u_c, -4.0, 4.0)
    v_c = jnp.clip(v_c, -4.0, 4.0)

    outside = (unclipped_xmin > 1.0) | (unclipped_xmax < -1.0) | (unclipped_ymin > 1.0) | (unclipped_ymax < -1.0)
    visible = any_valid & (~outside)
    f_vis = jnp.where(visible, 1.0, 0.0)

    w_b = jnp.clip(w_b, 0.0, 2.0)
    h_b = jnp.clip(h_b, 0.0, 2.0)

    return jnp.array([u_c, v_c, w_b, h_b, f_vis])

class KRTIAviaryJax(BaseAviaryJax):
    def __init__(self, curriculum_level=5,
                 crash_penalty=-100.0,
                 centering_weight=1.0,
                 attitude_penalty_weight=-1.0,
                 smoothness_penalty_weight=-0.5,
                 survival_reward=0.152,
                 **kwargs):
        from multi_drone_mujoco.envs.base_aviary import DRONE_PARAMS

        model_type = kwargs.get("drone_model", DroneModel.CF2X)
        DRONE_PARAMS[model_type]["mass"] = 3.500
        DRONE_PARAMS[model_type]["ixx"] = 0.080
        DRONE_PARAMS[model_type]["iyy"] = 0.080
        DRONE_PARAMS[model_type]["izz"] = 0.150
        DRONE_PARAMS[model_type]["arm_length"] = np.sqrt(0.130**2 + 0.200**2)
        DRONE_PARAMS[model_type]["kf"] = 0.002
        DRONE_PARAMS[model_type]["km"] = 8.0e-4
        DRONE_PARAMS[model_type]["collision_r"] = 0.235
        DRONE_PARAMS[model_type]["collision_h"] = 0.110
        DRONE_PARAMS[model_type]["prop_radius"] = 0.127

        self.curriculum_level = curriculum_level
        self.crash_penalty = crash_penalty
        self.centering_weight = centering_weight
        self.attitude_penalty_weight = attitude_penalty_weight
        self.smoothness_penalty_weight = smoothness_penalty_weight
        self.survival_reward = survival_reward

        # Curriculum settings bounds mapping
        if self.curriculum_level == 1:
            self.spawn_offset_x_max = 0.2
            self.spawn_yaw_max = 0.1
            self.min_delay = 0
            self.max_delay = 0
            self.yolo_noise_std = 0.0
            self.yolo_dropout_rate = 0.0
        elif self.curriculum_level == 2:
            self.spawn_offset_x_max = 0.3
            self.spawn_yaw_max = 0.15
            self.min_delay = 0
            self.max_delay = 1
            self.yolo_noise_std = 0.005
            self.yolo_dropout_rate = 0.01
        elif self.curriculum_level == 3:
            self.spawn_offset_x_max = 0.4
            self.spawn_yaw_max = 0.2
            self.min_delay = 0
            self.max_delay = 2
            self.yolo_noise_std = 0.01
            self.yolo_dropout_rate = 0.02
        elif self.curriculum_level == 4:
            self.spawn_offset_x_max = 0.5
            self.spawn_yaw_max = 0.25
            self.min_delay = 0
            self.max_delay = 2
            self.yolo_noise_std = 0.015
            self.yolo_dropout_rate = 0.03
        else: # Stage 5
            self.spawn_offset_x_max = 0.6
            self.spawn_yaw_max = 0.3
            self.min_delay = 0
            self.max_delay = 2
            self.yolo_noise_std = 0.02
            self.yolo_dropout_rate = 0.05

        # Lock limits globally
        self.max_xy_speed = 8.0
        self.max_z_speed = 3.0
        self.max_yaw_rate = 2.0

        super().__init__(drone_model=model_type, **kwargs)

        from multi_drone_mujoco.envs.base_aviary import _generate_aviary_xml
        from multi_drone_mujoco.examples.krti_arena import generate_krti_arena_xml

        init_xyzs = np.array([[0.0, 0.0, 0.25]] * self.num_drones)
        init_rpys = np.zeros((self.num_drones, 3))
        xml_str = _generate_aviary_xml(
            self.num_drones,
            self.drone_model,
            init_xyzs,
            init_rpys,
            obstacles=False,
            vision=True,
        )

        xml_str = xml_str.replace('size="10 10 0.05"', 'size="80 80 0.05"')
        xml_str = xml_str.replace('pos="0.17 9.26 0.0"', 'pos="0.17 21.26 0.0"')

        krti_xml = generate_krti_arena_xml()
        krti_xml = krti_xml.replace('contype="1"', 'contype="0"').replace(
            'conaffinity="1"', 'conaffinity="0"'
        )
        xml_str = xml_str.replace("</worldbody>", krti_xml + "\n  </worldbody>")

        self.mj_model = mujoco.MjModel.from_xml_string(xml_str)
        self.mj_model.opt.timestep = 1.0 / self.sim_freq

        self.mj_model.opt.solver = mujoco.mjtSolver.mjSOL_CG
        self.mj_model.opt.iterations = 8
        self.mj_model.opt.ls_iterations = 8
        self.mj_model.opt.disableflags |= mujoco.mjtDisableBit.mjDSBL_CONTACT

        self.cam_id = mujoco.mj_name2id(
            self.mj_model, mujoco.mjtObj.mjOBJ_CAMERA, "drone0_cam"
        )
        self.gate_body_id = mujoco.mj_name2id(
            self.mj_model, mujoco.mjtObj.mjOBJ_BODY, "gate_single_a"
        )
        self.drone_body_id = mujoco.mj_name2id(
            self.mj_model, mujoco.mjtObj.mjOBJ_BODY, "drone0"
        )

        self.mass = 3.500
        self.arm_length = jnp.sqrt(0.130**2 + 0.200**2)
        self.kf = 0.002
        self.km = 8.0e-4
        self.max_rpm = 100.0

        self.mj_model.body_mass[self.drone_body_id] = self.mass
        self.sys = mjx.put_model(self.mj_model)

        self.nominal_gate_pos = jnp.array([1.12156, 21.26, 1.0])
        self.nominal_drone_pos = jnp.array([0.92, 24.47, 1.0])

    @property
    def backend(self):
        return "mjx"

    @property
    def observation_size(self):
        return 46

    @property
    def action_size(self):
        return 4

    def _get_unbounded_yolo(self, d, gate_pos, noisy_gate_pos, rng_key, init_flag=False):
        cam_pos = d.cam_xpos[self.cam_id]
        cam_mat = d.cam_xmat[self.cam_id].reshape(3, 3)
        gate_mat = jnp.eye(3)

        gate_body_pos = gate_pos.at[0].add(-0.95156).at[2].set(0.0)
        yolo_raw = compute_unbounded_bbox_jax(
            gate_body_pos, gate_mat, cam_pos, cam_mat, 60.0, 320, 240
        )
        
        # Apply domain randomization during training:
        rng_noise, rng_drop = jax.random.split(rng_key)
        
        # BBox Noise: Add Gaussian noise to u, v, w, h to mimic YOLO prediction jitter
        noise = jax.random.normal(rng_noise, (4,)) * self.yolo_noise_std
        yolo_noisy = yolo_raw.at[:4].add(noise)
        
        # BBox Dropout: Randomly drop the visibility flag to 0 for 1-2 frames to simulate false negatives
        drop = jax.random.uniform(rng_drop) < self.yolo_dropout_rate
        yolo_obs = jnp.where(drop, yolo_noisy.at[4].set(0.0), yolo_noisy)
        
        return yolo_obs

    def reset(self, rng):
        rng, rng_g, rng_d, rng_m = jax.random.split(rng, 4)
        rng_g_dist, rng_g_pos = jax.random.split(rng_g, 2)

        # Full domain randomized stage 5 positioning always
        base_gate_y = 24.47 - jax.random.uniform(rng_g_dist, minval=6.0, maxval=9.0)
        nominal_gate_pos = jnp.array([1.12156, base_gate_y, 1.0])
        
        # Reduce lateral X offsets to make single-stage learning under delay solvable
        rng_g_pos_x, rng_g_pos_y = jax.random.split(rng_g_pos)
        g_offset_x = jax.random.uniform(rng_g_pos_x, (), minval=-self.spawn_offset_x_max, maxval=self.spawn_offset_x_max)
        g_offset_y = jax.random.uniform(rng_g_pos_y, (), minval=-1.0, maxval=1.0)
        g_offset = jnp.array([g_offset_x, g_offset_y])
        gate_pos = nominal_gate_pos.at[:2].add(g_offset)
        
        rng_d_x, rng_d_y = jax.random.split(rng_d)
        d_offset_x = jax.random.uniform(rng_d_x, (), minval=-self.spawn_offset_x_max, maxval=self.spawn_offset_x_max)
        d_offset_y = jax.random.uniform(rng_d_y, (), minval=-1.0, maxval=1.0)
        d_offset = jnp.array([d_offset_x, d_offset_y])
        drone_pos = self.nominal_drone_pos.at[:2].add(d_offset).at[2].set(1.0)
        dyaw = jax.random.uniform(rng_d, minval=-self.spawn_yaw_max, maxval=self.spawn_yaw_max)
        yaw = -jnp.pi / 2 + dyaw

        # Map noise (domain randomization)
        map_noise = jax.random.normal(rng_m, (3,)) * 0.20
        map_noise = map_noise.at[2].set(0.0)
        noisy_gate_pos = gate_pos + map_noise

        cr = jnp.cos(0.0)
        sr = jnp.sin(0.0)
        cp = jnp.cos(0.0)
        sp = jnp.sin(0.0)
        cy = jnp.cos(yaw / 2)
        sy = jnp.sin(yaw / 2)
        qw = cr * cp * cy + sr * sp * sy
        qx = sr * cp * cy - cr * sp * sy
        qy = cr * sp * cy + sr * sp * sy
        qz = cr * cp * sy - sr * sp * cy
        drone_quat = jnp.array([qw, qx, qy, qz])

        qpos = self.sys.qpos0
        qpos = qpos.at[0:3].set(drone_pos)
        qpos = qpos.at[3:7].set(drone_quat)

        qvel = jnp.zeros(self.sys.nv)
        d = mjx.make_data(self.sys)
        d = d.replace(qpos=qpos, qvel=qvel)
        d = mjx.step(self.sys, d)

        ctrl_state = init_controller_state(drone_pos, yaw)

        # Domain Randomization: Mass (+/- 10% around 3.5kg)
        rng, rng_mass = jax.random.split(rng)
        mass_rand = 3.500 * jax.random.uniform(rng_mass, (), minval=0.9, maxval=1.1)

        # Domain Randomization: Motor efficiency mapping (+/- 10%)
        rng, rng_kf, rng_km = jax.random.split(rng, 3)
        kf_rand = 0.002 * jax.random.uniform(rng_kf, (), minval=0.9, maxval=1.1)
        km_rand = 8.0e-4 * jax.random.uniform(rng_km, (), minval=0.9, maxval=1.1)

        def stabilize_step(i, val):
            cur_d, cur_ctrl_state = val
            pos = cur_d.qpos[0:3]
            quat = cur_d.qpos[3:7]
            vel = cur_d.qvel[0:3]
            omega = cur_d.qvel[3:6]
            
            rpm, next_ctrl = update_controller(
                self.dt,
                cur_ctrl_state,
                pos,
                quat,
                vel,
                omega,
                jnp.zeros(3),
                0.0,
                mass_rand,
                self.G,
                kf_rand,
                km_rand,
                self.arm_length,
                self.max_rpm,
            )
            
            forces = (rpm**2) * kf_rand
            torques = (rpm**2) * km_rand
            z_torque = -torques[0] + torques[1] - torques[2] + torques[3]
            
            mass_scale = 3.500 / mass_rand
            total_thrust = jnp.sum(forces) * mass_scale

            L = self.arm_length / jnp.sqrt(2.0)
            x_torque = (forces[0] + forces[1] - forces[2] - forces[3]) * L * mass_scale
            y_torque = (-forces[0] + forces[1] + forces[2] - forces[3]) * L * mass_scale
            z_torque = z_torque * mass_scale

            def body_fn(j, inner_d):
                rot_mat = inner_d.xmat[self.drone_body_id].reshape(3, 3)
                thrust_world = jnp.dot(rot_mat, jnp.array([0.0, 0.0, total_thrust]))
                torque_world = jnp.dot(rot_mat, jnp.array([x_torque, y_torque, z_torque]))

                force_torque = jnp.concatenate([thrust_world, torque_world])
                inner_d = inner_d.replace(
                    xfrc_applied=inner_d.xfrc_applied.at[self.drone_body_id].set(force_torque)
                )

                inner_d = mjx.step(self.sys, inner_d)
                return inner_d

            next_d = jax.lax.fori_loop(0, self.sim_steps_per_ctrl, body_fn, cur_d)
            return next_d, next_ctrl

        d, ctrl_state = jax.lax.fori_loop(0, 40, stabilize_step, (d, ctrl_state))

        # Domain Randomization: Action Delay (Curriculum parameterized delay buffer)
        rng, rng_delay = jax.random.split(rng)
        action_delay = jax.random.randint(rng_delay, (), minval=self.min_delay, maxval=self.max_delay + 1)
        action_buffer = jnp.zeros((3, 4))

        rng, rng_yolo = jax.random.split(rng)
        yolo_obs = self._get_unbounded_yolo(d, gate_pos, noisy_gate_pos, rng_yolo, init_flag=True)

        yolo_history = jnp.tile(yolo_obs[jnp.newaxis, :], (4, 1))

        info = {
            "gate_pos": gate_pos,
            "noisy_gate_pos": noisy_gate_pos,
            "ctrl_state": ctrl_state,
            "step": 0,
            "rng": rng,
            "last_yolo": yolo_obs,
            "prev_action": jnp.zeros(4),
            "action_delay": action_delay,
            "action_buffer": action_buffer,
            "yolo_history": yolo_history,
            "mass": mass_rand,
            "kf": kf_rand,
            "km": km_rand,
        }

        obs = self._get_obs(d, info)

        metrics = {
            "crashed": jnp.zeros(()),
            "cleared_gate": jnp.zeros(()),
            "gate_collided": jnp.zeros(()),
            "gate_distance": jnp.linalg.norm(gate_pos - drone_pos),
            "reward_progress": jnp.zeros(()),
            "reward_centering": jnp.zeros(()),
            "reward_orientation": jnp.zeros(()),
            "reward_speed": jnp.zeros(()),
            "reward_attitude": jnp.zeros(()),
            "reward_smooth": jnp.zeros(()),
            "reward_terminal": jnp.zeros(()),
            "reward_out_of_range": jnp.zeros(()),
        }

        return State(
            d, obs, jnp.zeros(()), jnp.zeros((), dtype=jnp.float32), metrics, info
        )

    def _get_obs(self, d, info):
        drone_quat = d.qpos[3:7]
        vx_world, vy_world, vz_world = d.qvel[0:3]

        w, x, y, z = drone_quat
        roll = jnp.arctan2(2 * (w * x + y * z), 1 - 2 * (x * x + y * y))
        pitch = jnp.arcsin(jnp.clip(2 * (w * y - z * x), -1.0, 1.0))
        yaw = jnp.arctan2(2 * (w * z + x * y), 1 - 2 * (y * y + z * z))

        cos_y, sin_y = jnp.cos(yaw), jnp.sin(yaw)

        vel_body = jnp.array([
            vx_world * cos_y + vy_world * sin_y,
            -(-vx_world * sin_y + vy_world * cos_y),
            -vz_world,
        ])

        vel_body_scaled = jnp.clip(vel_body / 10.0, -1.0, 1.0)
        omega_body = d.qvel[3:6]
        omega_body_scaled = jnp.clip(omega_body / jnp.pi, -1.0, 1.0)

        attitude = jnp.array([
            jnp.sin(roll), jnp.cos(roll),
            jnp.sin(pitch), jnp.cos(pitch),
            jnp.sin(yaw), jnp.cos(yaw)
        ])
        
        agl = d.qpos[2]
        time_remaining = (450.0 - info["step"]) / 450.0

        return jnp.concatenate([
            vel_body_scaled,
            omega_body_scaled,
            attitude,
            jnp.array([agl]),
            jnp.array([time_remaining]),
            info["yolo_history"].flatten(),
            info["action_buffer"].flatten(),
        ])

    def step(self, state, action):
        MAX_STEPS = 450
        
        # Domain Randomization: Action Delay logic
        action_buffer = jnp.concatenate([state.info["action_buffer"][1:], action[jnp.newaxis, :]], axis=0)
        delayed_action = action_buffer[2 - state.info["action_delay"]]
        
        vx_body = delayed_action[0] * self.max_xy_speed
        vy_body = -delayed_action[1] * self.max_xy_speed
        vz_body = -delayed_action[2] * self.max_z_speed
        yaw_rate = -delayed_action[3] * self.max_yaw_rate
        
        target_vel = jnp.array([vx_body, vy_body, vz_body])
        d = state.pipeline_state

        # Domain Randomization: Retrieve randomized physical dynamics
        kf_val = state.info["kf"]
        km_val = state.info["km"]
        mass_val = state.info["mass"]

        rpm, next_ctrl_state = update_controller(
            self.dt,
            state.info["ctrl_state"],
            d.qpos[0:3],
            d.qpos[3:7],
            d.qvel[0:3],
            d.qvel[3:6],
            target_vel,
            yaw_rate,
            mass_val,
            self.G,
            kf_val,
            km_val,
            self.arm_length,
            self.max_rpm,
        )

        forces = (rpm**2) * kf_val
        torques = (rpm**2) * km_val
        z_torque = -torques[0] + torques[1] - torques[2] + torques[3]

        mass_scale = 3.500 / mass_val
        total_thrust = jnp.sum(forces) * mass_scale

        L = self.arm_length / jnp.sqrt(2.0)
        x_torque = (forces[0] + forces[1] - forces[2] - forces[3]) * L * mass_scale
        y_torque = (-forces[0] + forces[1] + forces[2] - forces[3]) * L * mass_scale
        z_torque = z_torque * mass_scale

        def body_fn(i, cur_d):
            rot_mat = cur_d.xmat[self.drone_body_id].reshape(3, 3)
            thrust_world = jnp.dot(rot_mat, jnp.array([0.0, 0.0, total_thrust]))
            torque_world = jnp.dot(rot_mat, jnp.array([x_torque, y_torque, z_torque]))

            force_torque = jnp.concatenate([thrust_world, torque_world])
            cur_d = cur_d.replace(
                xfrc_applied=cur_d.xfrc_applied.at[self.drone_body_id].set(force_torque)
            )

            cur_d = mjx.step(self.sys, cur_d)
            return cur_d

        d = jax.lax.fori_loop(0, self.sim_steps_per_ctrl, body_fn, d)

        # Distances for Potential-Based Progress Reward
        dist_before = jnp.linalg.norm(
            state.info["gate_pos"] - state.pipeline_state.qpos[0:3]
        )
        dist_after = jnp.linalg.norm(state.info["gate_pos"] - d.qpos[0:3])

        w_q, x_q, y_q, z_q = d.qpos[3], d.qpos[4], d.qpos[5], d.qpos[6]
        roll = jnp.arctan2(2 * (w_q * x_q + y_q * z_q), 1 - 2 * (x_q * x_q + y_q * y_q))
        pitch = jnp.arcsin(jnp.clip(2 * (w_q * y_q - z_q * x_q), -1.0, 1.0))
        yaw = jnp.arctan2(2 * (w_q * z_q + x_q * y_q), 1 - 2 * (y_q * y_q + z_q * z_q))

        # Compute the new YOLO observation at step t
        rng, subrng = jax.random.split(state.info["rng"])
        yolo_obs = self._get_unbounded_yolo(d, state.info["gate_pos"], state.info["noisy_gate_pos"], subrng)
        
        # Slide the YOLO history window
        new_yolo_history = jnp.concatenate([state.info["yolo_history"][1:], yolo_obs[jnp.newaxis, :]], axis=0)

        new_info = dict(state.info)
        new_info["ctrl_state"] = next_ctrl_state
        new_info["step"] = state.info["step"] + 1
        new_info["rng"] = rng
        new_info["action_buffer"] = action_buffer
        new_info["prev_action"] = action
        new_info["yolo_history"] = new_yolo_history
        new_info["last_yolo"] = yolo_obs

        obs = self._get_obs(d, new_info)

        # --- DENSE STEP REWARDS (SECTION 4.1) ---
        
        # 1. Potential-Based Progress Reward: R_progress = 0.99 * Phi(s') - Phi(s)
        r_progress = 10.0 * (dist_before - 0.99 * dist_after)

        # 2. Image Alignment Reward: R_align = -k1 * (u^2 + v^2)
        # current u, v coordinates are at index 29, 30 of new observation
        u_c = obs[29]
        v_c = obs[30]
        f_vis = obs[33]
        # k1 set to 0.2 and active unboundedly to provide continuous steering gradients
        r_align = -0.2 * (jnp.square(u_c) + jnp.square(v_c))

        # 3. Visibility Penalty: P_vis = 0.5 if visibility == 0 else 0
        p_vis = jnp.where(f_vis == 0.0, 0.5, 0.0)

        # 4. Control Smoothness Penalty: P_ctrl = k2 * ||a_t - a_{t-1}||^2
        action_change = action - state.info["prev_action"]
        # Smoothness weight scaled down to 0.05
        p_ctrl = 0.05 * jnp.sum(jnp.square(action_change))

        # 5. Conditional Tilt Penalty: only penalize roll/pitch when forward command (action[0]) is low
        is_low_cmd = jnp.abs(action[0]) < 0.2
        p_tilt = jnp.where(
            is_low_cmd,
            1.0 * (jnp.square(roll) + jnp.square(pitch)),
            0.0
        )

        # Constant survival reward to offset negative penalties and incentivize staying alive
        r_survival = 1.5

        step_reward = r_progress + r_align - p_vis - p_ctrl - p_tilt + r_survival

        # --- TERMINAL CRASH / COLLISION / PASS LOGIC (SECTION 4.2) ---
        crossed_plane = (state.pipeline_state.qpos[1] > state.info["gate_pos"][1]) & (
            d.qpos[1] <= state.info["gate_pos"][1]
        )
        
        within_gate_frame = (
            (d.qpos[0] >= state.info["gate_pos"][0] - 0.88) & 
            (d.qpos[0] <= state.info["gate_pos"][0] + 0.88) & 
            (d.qpos[2] >= state.info["gate_pos"][2] - 0.82) &
            (d.qpos[2] <= state.info["gate_pos"][2] + 0.82)
        )

        in_gate_y_zone = jnp.abs(d.qpos[1] - state.info["gate_pos"][1]) < 0.25

        hits_left_post = (
            (d.qpos[0] < state.info["gate_pos"][0] - 0.70) & 
            (d.qpos[0] > state.info["gate_pos"][0] - 1.10) & 
            (d.qpos[2] < 1.85)
        )

        hits_right_post = (
            (d.qpos[0] > state.info["gate_pos"][0] + 0.70) & 
            (d.qpos[0] < state.info["gate_pos"][0] + 1.10) & 
            (d.qpos[2] < 1.85)
        )

        hits_top_arch = (
            (d.qpos[0] >= state.info["gate_pos"][0] - 1.10) & 
            (d.qpos[0] <= state.info["gate_pos"][0] + 1.10) & 
            (d.qpos[2] > 1.77) &
            (d.qpos[2] < 1.99)
        )

        is_gate_crash = in_gate_y_zone & (hits_left_post | hits_right_post | hits_top_arch)

        is_generic_crash = (
            (d.qpos[2] < 0.08)
            | (d.qpos[2] > 4.5)
            | (jnp.abs(roll) > jnp.pi / 2.5)
            | (jnp.abs(pitch) > jnp.pi / 2.5)
        )

        velocity_forward = d.qvel[1] < 0.0
        cleared_gate = crossed_plane & within_gate_frame & velocity_forward & (~is_gate_crash) & (~is_generic_crash)
        
        # Missing the gate: crossing the plane but not through the frame
        is_gate_miss = crossed_plane & (~within_gate_frame) & velocity_forward & (~is_generic_crash)

        # Terminal rewards: success reward adds the remaining survival reward to prevent truncation penalty
        success_reward = 300.0 + r_survival * (MAX_STEPS - new_info["step"])
        terminal_reward = jnp.where(cleared_gate, success_reward, 0.0)
        terminal_reward += jnp.where(is_gate_crash | is_generic_crash | is_gate_miss, -100.0, 0.0)

        reward = step_reward + terminal_reward

        done = cleared_gate | is_gate_crash | is_generic_crash | is_gate_miss
        done = (done | (new_info["step"] >= MAX_STEPS)).astype(jnp.float32)

        metrics = dict(state.metrics)
        metrics.update(
            {
                "crashed": (is_generic_crash | is_gate_crash | is_gate_miss).astype(jnp.float32),
                "cleared_gate": cleared_gate.astype(jnp.float32),
                "gate_collided": is_gate_crash.astype(jnp.float32),
                "gate_distance": dist_after,
                "reward_progress": r_progress,
                "reward_centering": r_align,
                "reward_orientation": jnp.zeros(()),
                "reward_speed": jnp.zeros(()),
                "reward_attitude": -p_tilt,
                "reward_smooth": -p_ctrl,
                "reward_terminal": terminal_reward,
                "reward_out_of_range": -p_vis,
            }
        )

        return state.replace(
            pipeline_state=d,
            obs=obs,
            reward=reward,
            done=done,
            metrics=metrics,
            info=new_info,
        )